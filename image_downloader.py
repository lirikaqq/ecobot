...
def download_image(url):
    try:
        ...
        picture_tag = soup.find('picture', class_='info-block-image-wrapper')
        if picture_tag:
        ...
        # Если <picture> не найден, попробуем найти <img> с классом 'picture-image'
        img_tag = soup.find('img', class_='picture-image')
        if img_tag and 'src' in img_tag.attrs:
            image_url = img_tag['src']
